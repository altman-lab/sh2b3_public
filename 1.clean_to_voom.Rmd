...
meta2.filtered <- bind_rows(exp1_sample_key, exp2_sample_key) %>% 
  dplyr::right_join(meta1.filter) %>% 
...
counts.filter.pc.combat <- counts.filter.pc %>%
  ...
  as.matrix() %>%
  # batch correction
  sva::ComBat_seq(., batch = meta2.filtered$Expt,
...
BIGpicture::plot_pca(counts.filter.pc.combat, meta = meta2.filtered, vars = "outlier",
...
meta2.filtered.ord <- meta2.filtered %>%
  dplylr::arrange(libID)

counts.filter.pc.combat.ord <- counts.filter.pc.combat %>%
  dplyr::select(all_of(meta2.filtered.ord$libID)) %>% 
  tibble::rownames_to_column() %>% 
  dplyr::arrange(rowname) %>% 
  tibble::column_to_rownames()

identical(meta2.filtered.ord$libID,
          colnames(counts.filter.pc.combat.ord)) # TRUE

dat <- DGEList(
  counts = as.matrix(counts.filter.pc.combat.ord),
  samples = meta2.filtered.ord,
