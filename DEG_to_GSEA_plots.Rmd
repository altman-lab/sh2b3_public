...
exp1.E <- as.data.frame(voom.exp1$E) %>% 
  ...
  left_join(voom.exp1$targets %>% select(libID, Genotype, Time, Animal)) %>% 
  mutate(name = paste(Genotype, Time, Animal)) %>% 
  dplyr::select(-c(libID, Genotype, Time, Animal)) %>% 
  pivot_wider() %>% 
  #Filter to significant genes
  filter(rowname %in% exp1_sig_genes) %>% 
  #Rename genes to symbols
  left_join(voom.exp1$genes, by=c("rowname"="ensembl_gene_id")) %>% 
  unnest(symbol) %>% 
  select(-rowname) %>% 
...
exp2.E <- as.data.frame(voom.exp2$E) %>% 
  ...
  #Rename libraries
  pivot_longer(-rowname, names_to = "libID") %>% 
  left_join(voom.exp2$targets %>% select(libID, Genotype, Time, Animal)) %>% 
  mutate(name = paste(Genotype, Time, Animal)) %>% 
  dplyr::select(-c(libID, Genotype, Time, Animal)) %>% 
  pivot_wider() %>% 
  #Filter to significant genes
  filter(rowname %in% exp1_sig_genes) %>% 
  #Rename genes to symbols
  left_join(voom.exp2$genes, by=c("rowname"="ensembl_gene_id")) %>% 
  unnest(symbol) %>% 
  select(-rowname) %>% 
  column_to_rownames("symbol") %>%
  as.matrix()
...
exp2_H_enr_df <- exp2_Hallmark %>%
  ...
  filter(group_in_pathway > 5 & FDR < 0.5)
...
enr_all <- SEARchways::BIGprofiler(
  gene_list = list("0hr DOWN" = bp_0hr_dn,
                   "0hr UP" = bp_0hr_up,
                   "24hr DOWN" = bp_24hr_dn,
                   "24hr UP" = bp_24hr_up,
                   "24hr+anti-IL2 DOWN" = bp_24hrIL2_dn,
                   "24hr+anti-IL2 UP" = bp_24hrIL2_up),
  ID = "ENSEMBL",
  species = "mouse",
  category = "H")
...
exp2_enr_bp_df <- enr_all %>%
  ...
  filter(group %in% c("24hr DOWN", "24hr UP", 
                      "24hr+anti-IL2 DOWN", "24hr+anti-IL2 UP")) %>% 
  mutate(pathway = gsub("HALLMARK_", "", pathway)) %>%
  mutate(pathway = gsub("_", " ", pathway)) %>% 
  mutate(contrast = factor(group, 
                         levels = c("24hr DOWN", "24hr UP", 
                                      "24hr+anti-IL2 DOWN", "24hr+anti-IL2 UP")))
